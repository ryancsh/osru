...
impl AudioManager{
    ...
    pub fn track_volume(&self, id: usize) -> f32 {self.sources.get(id).unwrap().track_volume()}

    pub fn sleep_until_end(&self){
        for sink in self.sinks.iter(){sink.sleep_until_end();}
    }
    ...
    pub fn master_volume(&self) -> f32 { self.sinks[0].master_volume()}
...
impl AudioSource{
    pub fn new(filename: &str) -> AudioSource{
        AudioSource{
            audiofile: Arc::new(AudioFile::new(filename)),
            current_pos: 0,
        }
    }

    pub fn track_volume(&self) -> f32 {self.audiofile.volume()}
    pub fn channels(&self) -> u16 {self.audiofile.channels()}
    pub fn sample_rate(&self) -> u32 {self.audiofile.sample_rate()}
    pub fn sample_at(&self, position: usize)-> Option<i16>{
        self.audiofile.sample_at(position)
    }
    pub fn len(&self) -> usize{self.audiofile.len()}
    pub fn current_pos(&self) -> usize {self.current_pos}
    pub fn set_current_pos(&mut self, value: usize){self.current_pos = value}
}

impl Clone for AudioSource{
    fn clone(&self) -> AudioSource{
        AudioSource{
            audiofile: Arc::clone(&self.audiofile),
            current_pos: 0,
        }
    }
}
...
impl Source for AudioSource{
    ...
    fn total_duration(&self) -> Option<Duration>{
        ...
        Some(
            Duration::new(duration_s as u64, duration_ns as u32)
        )
    }
}
...
impl SinkWrapper{
    ...
    pub fn sleep_until_end(&self){self.sink.sleep_until_end()}
...
mod tests{
    ...
    fn test_normalize_volume(){
        fn test_vol(message: &str, samples: &Vec<i16>) {
            ...
            assert_eq!((AUDIO_REFERENCE_POWER as f64 * count as f64/ sum as f64) as f32 * DEFAULT_TRACK_VOLUME, AudioManager::normalize_volume(samples));
        }
