   ...
   pub fn next_update(&mut self) -> Option<InputUpdate> {
      'next_event: loop {
         if let Some(event) = self.event_pump.poll_event() {
            self.prev_snapshot.copy(&self.cur_snapshot);
            match event {
               Event::Quit { .. } | Event::KeyDown { keycode: Some(Keycode::Escape), .. } => {
                  self.keep_running = false;
                  return None;
               ...
               Event::MouseMotion { timestamp: t, x: x_pos, y: y_pos, .. } => {
                  self.cur_snapshot.time = self.reference_time().elapsed_sdl_time(SdlTime(t as usize));
                  self.cur_snapshot.mouse_position = Pixels(x_pos as f64, y_pos as f64);
               }
               /*
               ...
               */
               Event::KeyDown { scancode: Some(code), repeat: false, timestamp: t, .. } => {
                  use Scancode::*;
                  let time = self.reference_time().elapsed_sdl_time(SdlTime(t as usize));
                  match code {
                     Z => {
                        self.cur_snapshot.time = time;
                        self.cur_snapshot.K1 = true;
                     }
                     X => {
                        self.cur_snapshot.time = time;
                        self.cur_snapshot.K2 = true;
                     }
                     _ => (),
                  }
               }
               Event::KeyUp { scancode: Some(code), repeat: false, timestamp: t, .. } => {
                  ...
                  match code {
                     Z => {
                        self.cur_snapshot.time = time;
                        self.cur_snapshot.K1 = false;
                     }
                     X => {
                        self.cur_snapshot.time = time;
                        self.cur_snapshot.K2 = false;
                     }
                     _ => (),
                  }
               }
            ...
            return Some(InputUpdate::new(&self.prev_snapshot, &self.cur_snapshot));
         } else {
            return None;
         }
      }
